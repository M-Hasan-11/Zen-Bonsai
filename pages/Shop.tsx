              ...
              {PRODUCTS.map((product) => (
                <Link to={`/product/${product.id}`} key={product.id} className="flex flex-col group cursor-pointer">
                  <div className="relative aspect-[4/5] rounded-xl overflow-hidden bg-surface-dark mb-4">
                    ...
                    <button onClick={(e) => handleQuickAdd(e, product)} aria-label={`Add ${product.name} to cart`} className="absolute bottom-4 right-4 size-10 bg-primary text-white rounded-full flex items-center justify-center translate-y-14 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 focus:opacity-100 focus:translate-y-0 transition-all duration-300 shadow-lg hover:bg-white hover:text-primary z-10">
