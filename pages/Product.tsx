                    ...
                    <div className="flex-1 flex justify-between">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <div className="flex items-center h-8 bg-background-dark rounded border border-white/10">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} aria-label={`Decrease quantity of ${item.name}`} className="px-2 text-text-muted hover:text-white">-</button>
                          <span className="w-8 text-center text-white text-sm">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} aria-label={`Increase quantity of ${item.name}`} className="px-2 text-text-muted hover:text-white">+</button>
                        </div>
                      </div>
                    </div>
                  </div>
