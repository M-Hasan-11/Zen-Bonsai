          ...
          <div className="flex flex-col py-2">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <div className="flex items-center h-14 bg-surface-dark rounded-lg border border-white/10 w-full sm:w-auto">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} aria-label="Decrease quantity" className="px-4 text-text-muted hover:text-white transition-colors h-full">-</button>
                ...
                <button onClick={() => setQuantity(quantity + 1)} aria-label="Increase quantity" className="px-4 text-text-muted hover:text-white transition-colors h-full">+</button>
              </div>
                  ...
                  <div key={item.id} className="flex gap-4 p-4 bg-surface-dark border border-[#2f3231] rounded-xl">
                    ...
                    <div className="flex-1 flex justify-between">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <button onClick={() => removeFromCart(item.id)} aria-label="Remove item" className="text-text-muted hover:text-red-400"><span className="material-symbols-outlined">delete</span></button>
                        <div className="flex items-center h-8 bg-background-dark rounded border border-white/10">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} aria-label="Decrease quantity" className="px-2 text-text-muted hover:text-white">-</button>
                          <span className="w-8 text-center text-white text-sm">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} aria-label="Increase quantity" className="px-2 text-text-muted hover:text-white">+</button>
                        </div>
                      </div>
                    </div>
                  </div>
