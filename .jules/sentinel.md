...
## 2026-01-24 - Schema Validation in NoSQL
**Vulnerability:** Lack of schema enforcement in NoSQL allows clients to inject malformed data (e.g., negative totals, incorrect status).
**Learning:** Firestore rules can and should be used as a schema validator for write operations to ensure data integrity.
**Prevention:** Explicitly type-check and value-check fields (like `total`, `status`, `items`) in `allow create` rules.
